...
**Note on SonarQube Verification:**
SonarQube scan to confirm issue resolution is performed in Phase 30 (Final SonarQube Scan). This phase focuses on the code changes; Phase 30 validates the metrics improvement.

**Note on Macro Count (5 vs ~111 issues):**
The research identified ~111 SonarQube macro issues, but most fall into won't-fix categories:
- ~50 Resource IDs (IDD_*, IDC_*, etc.) - required by resource compiler
- ~10 Windows header config macros (WIN32_NO_STATUS, etc.) - configure SDK behavior
- ~15 Function-like macros using __FILE__, __LINE__, ##, # - no C++ equivalent
- ~10 Include guards - standard pattern
- ~6 Windows SDK macros in cardmod.h - external file
- ~15 Already converted in prior phases (AUTHENTICATIONPACKAGENAME, WINEVENT_LEVEL_*, MAX_*_LENGTH)

The 5 macros in these plans are the remaining truly convertible ones. All won't-fix categories are documented in Task 3.
</success_criteria>
